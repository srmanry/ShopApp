...
class TailorYourJourWidget extends StatelessWidget {
  const TailorYourJourWidget({super.key});
    ...
    return GetBuilder<AuthController>(builder: (authController) {
      return ListView(
        ...
        children: [
          Wrap(
            children:
                List.generate(authController.locations.length, (index) {
                  var data =  authController.locations.elementAt(index);
                  String placeName = data.placeName;
                  bool isSelect = data.isSelect;
                  return CustomChip(name: placeName, isSelect: isSelect, onSelected: () => authController.selectJourneyLocation(index: index));
                }),
          )
        ],
      );
